...
ff = np.fromfile(r'Pictodrawing\nrfsample.jpg', np.uint8)
img = cv2.imdecode(ff, cv2.IMREAD_UNCHANGED)
img = cv2.resize(img, dsize=(900, 600), fx=1.0, fy=1.0, interpolation=cv2.INTER_LINEAR)
...
while True:
  ...
  cartoon_image = cv2.stylization(img, sigma_s=sigma_s_value, sigma_r=sigma_r_value)
  
  cv2.imshow("Trackbar Windows", cartoon_image)
  
  cv2.imwrite(r'Pictodrawing\output_cartoon_image.jpg', cartoon_image)
